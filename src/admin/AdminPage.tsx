...
function AdminPage() {
  ...
  const fetchUserScore = async () => {
    ...
    try {
      const response = await fetch(`https://server500.actoria.top/api/scores/${userId}`);
  ...
  const submitQuestion = async (questionId: string) => {
    ...
    try {
      // 调用游戏引擎API
      const response = await fetch('https://server500.actoria.top/api/answers', {
        method: 'POST',
        headers: {
      ...
      if (result.code === 0) {
        ...
        alert(`提交成功！${question?.scoring ? `本题得分: ${result.data?.question_score || 0}分，总分: ${result.data?.total_score || 0}分` : ''}`);
      } else {
      ...
      case 'text':
        return (
          <input
            ...
            placeholder={question.id === 'c3' ? '例如: 25年6月' : `请输入${question.title}`}
  ...
  return (
    <div className="admin-container" style={{ backgroundColor: '#fefefe', minHeight: '100vh' }}>
      <div className="admin-header">
        ...
        <p className="instruction">请逐一回答以下问题，每题回答完成后点击对应的提交按钮</p>
      ...
      <div className="questions-container">
        <div className="questions-table">
          ...
          {questions.map((question) => {
            ...
            return (
              <div key={question.id} className={`table-row ${isSubmitted ? 'submitted' : ''} ${question.scoring ? 'scoring-question' : 'non-scoring-question'}`}>
                <div className="table-cell question-cell">
                  <div className="question-header-info">
                    <div className="question-id">ID: {question.id}</div>
                    <div className="question-title">{question.title}</div>
                    {question.scoring && <span className="scoring-badge">计分题</span>}
